...
namespace MESSAGE{
	...
	const int LATESTQUOTE_SZPEX	=	82;//深圳石油交易所行情
	...
	const int BAIJIA_INDEX_REQUEST=118;//百家首页资讯
	const int BAIJIA_INDEX_PUSH=119;//百家首页资讯
	const int QIANKUN_REQUEST=120;//乾坤线信号请求
	const int CALC_SIGNAL_TAIJI = 150;//太极图信号
	const int CALC_SIGNAL_QIANKUN = 151;//乾坤线信号推送
	const int LSPFIVE_REQUEST=152;
	const int LSPFIVE_PUSH=153;
	const int CALC_SIGNAL_FORMULAR = 154;//指标信号推送
	const int KLINEDATA_NOTIFY = 155;//k线数据生成通知
	const int QIANKUN_MAP_PUSH = 156;         //乾坤图信号推送
	const int QIANKUN_MAP_REQUEST = 157;//乾坤图数据生成通知
	const int BY_ARRAY_PUSH = 158;         //博弈矩阵信号推送
        const int BY_ARRAY_REQUEST = 159;//博弈矩阵数据请求
	const int SECURITYLIST2	= 200;//新码表请求
	const int CLEARMARKET = 201;//市场开盘通知
	const int CLEARSECURITY = 202;//市场下某个品种开盘通知
	const int LOGONSUCCESS = 254;//通知PC客户端登录成功
